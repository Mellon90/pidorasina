...
class Office(Room):
    def __init__(self,name, number, edges=[], availability = False, r_door = False, l_door = False, energy_count = 100, use_energy = 0.0):
        self.r_door = r_door
        self.l_door = l_door
...
class Animatronic:
    ...
    def move_trajectory(self):
        while True:
       # выбирает рандомно траекторию.
            real_trajectory = random.choice(self.trajectory)
            for i in rooms:
                if i.number == self.real_location:
                   #real нахождение анима
...
def start_game(animatronics, rooms, office):
    ...
    def update_energy():
        while True:
            office.energy_count = office.energy_count - 0.1
            print(office.energy_count.__round__())
            time.sleep(1)
